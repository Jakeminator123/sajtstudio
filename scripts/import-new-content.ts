...
console.log(
  `✅ Merged ${Object.keys(newContent).length} entries from new.json into content-cache.json`
)
...
const transaction = contentDb.transaction(() => {
  for (const [key, entry] of Object.entries(merged)) {
    const result = updateStmt.run(key, entry.type, entry.section, entry.value, entry.label)
    if (result.changes > 0) {
      updatedCount++
    }
  }
