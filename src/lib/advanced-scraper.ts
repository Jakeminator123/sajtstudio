...
function extractLinks($: CheerioAPI, baseUrl: string): { internal: string[]; external: string[] } {
...
function scoreUrl(url: string, priorityPatterns: string[]): number {
  ...
  for (let i = 0; i < priorityPatterns.length; i++) {
    if (urlPath === priorityPatterns[i] || urlPath === priorityPatterns[i] + '/') {
...
async function scrapePage(url: string, timeout: number = 10000): Promise<PageContent | null> {
  ...
  try {
    ...
    // Count links
    const { internal: internalLinks, external: externalLinks } = extractLinks($, normalizedUrl)
    ...
    // Create text preview
    const textPreview = limitedText.substring(0, 300) + (limitedText.length > 300 ? '...' : '')
...
export async function scrapeMultiplePages(
  ...
  const additionalPagePromises = scoredUrls.map(({ url }) => scrapePage(url, opts.timeout))

  const additionalPageResults = await Promise.all(additionalPagePromises)
  const additionalPages = additionalPageResults.filter((page) => page !== null) as PageContent[]
