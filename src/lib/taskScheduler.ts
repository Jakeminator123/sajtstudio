...
export function measureTask<T>(
  ...
  if (duration > threshold && typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
    console.warn(`[Performance] Task "${name}" took ${duration.toFixed(2)}ms (threshold: ${threshold}ms)`);
  }
