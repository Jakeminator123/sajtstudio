...
export default function HeroAnimation() {
  ...
  const scrollToTechSection = useCallback(() => {
    ...
    if (parentWrapper && parentWrapper.nextElementSibling instanceof HTMLElement) {
  ...
  // Auto-play explosion when scroll reaches the moment images start exploding (~0.45)
  useEffect(() => {
    if (explosionAutoPlay) return;

    const explosionDuration = 3000; // 3 seconds
    const whiteoutScrollDelay = 800; // wait for white screen to fully settle before scrolling
    let whiteoutCheckRaf: number | null = null;
    const explosionStartProgress = 0.45;
  ...
  }, [explosionAutoPlay, mediaScrollProgress, autoProgressMotion, scrollToTechSection]);
  ...
  const explosionGlowOpacity = useTransform(smoothMediaProgress, [0.45, 0.55, 0.65, 0.75], [0, 0.6, 0.4, 0.1]);
  const explosionParticlesOpacity = useTransform(smoothMediaProgress, [0.5, 0.55, 0.6], [0, 0.3, 0]);
  const imagesContainerZIndex = useTransform(smoothMediaProgress, [0, 0.5, 0.65], [20, 60, 20]);

  // 3D transforms for images - only for images 1 and 3 that fly towards screen
  const image3DRotateX = useTransform(smoothMediaProgress, [0, 0.5, 0.55, 0.65], [0, 0, 10, 20]);
  const image3DRotateY = useTransform(smoothMediaProgress, [0, 0.5, 0.55, 0.65], [0, 0, 5, 10]);
  const image3DZ = useTransform(smoothMediaProgress, [0, 0.5, 0.55, 0.65], [0, 0, 500, 800]);
  ...
  const imageZIndexExplosion = useTransform(smoothMediaProgress, [0, 0.5, 0.65], [20, 70, 20]);
  const imagePointerEventsValue = useTransform(smoothMediaProgress, [0, 0.5, 0.65], [1, 0, 0]);
  const imagePointerEventsExplosion = useTransform(imagePointerEventsValue, (val) => val === 1 ? 'auto' : 'none');

  // Additional transforms for video effects
  const smokeLayerOpacity = useTransform(videoRedTint, [0, 1], [0, 0.5]);
  // White fade starts earlier and stays longer for Matrix text effect
  const whiteFadeOverlayOpacity = useTransform(smoothMediaProgress, [0.75, 0.85, 0.95, 1], [0, 0.3, 0.8, 1]);
  const whiteFadeTransitionOpacity = useTransform(scrollYProgress, [0.85, 0.95, 1], [0, 0.8, 1]);
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <motion.h2
            style={mounted ? { color: headingColor } : { color: "rgba(255, 255, 255, 1)" }}
              ...
              return (
                <motion.div
                  ...
                  style={mounted ? {
                    ...
                    rotateX: index === 1 || index === 3 ? image3DRotateX : imageNo3DRotateX,
                    rotateY: index === 1 || index === 3 ? image3DRotateY : imageNo3DRotateY,
