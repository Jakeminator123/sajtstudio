...
export default function HeroAnimation() {
  ...
  const [primaryDebrisPositions, setPrimaryDebrisPositions] = useState<DebrisSnapshot[] | null>(
    null
  )
  const [secondaryDebrisPositions, setSecondaryDebrisPositions] = useState<DebrisSnapshot[] | null>(
    null
  )
  ...
  const redPadX = useTransform(smoothMediaProgress, redPadKeyframes, [-420, -240, -140, -80, -60])
  const redPadY = useTransform(smoothMediaProgress, redPadKeyframes, [-320, -140, 20, 190, 250])
  ...
  const redPadRotate = useTransform(smoothMediaProgress, redPadKeyframes, [32, 24, 16, 9, 4])
  const redPadSkewX = useTransform(smoothMediaProgress, redPadKeyframes, [10, 8, 6, 4, 3])
  ...
  const vrPortalPulse = useTransform(smoothMediaProgress, [0.45, 0.6, 0.75, 1], [0, 1, 0.75, 0.6])
  ...
  const imagesContainerZIndex = useTransform(smoothMediaProgress, [0, 0.6, 0.75], [20, 60, 20])

  // Z-index and pointer events for images during explosion
  const imageZIndexExplosion = useTransform(smoothMediaProgress, [0, 0.6, 0.75], [20, 70, 20])
  const imagePointerEventsValue = useTransform(smoothMediaProgress, [0, 0.6, 0.75], [1, 0, 0])
  const imagePointerEventsExplosion = useTransform(imagePointerEventsValue, (val) =>
    val === 1 ? 'auto' : 'none'
  )
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          >
            Scrolla för att se hur videon expanderar, bilderna exploderar och allt förvandlas till
            spillror
          </motion.p>
        </motion.div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 rounded-lg overflow-hidden shadow-2xl border-2 border-accent/20 relative">
                  ...
                  {mounted && !videoError && (
                    ...
                    >
                      <source src="/videos/unclear_md.mp4" type="video/mp4" />
