...
export default function ServicesSection() {
  ...
  // Always call hooks - never conditionally
  const accentGradientOpacity = useTransform(scrollYProgress, [0, 0.5, 1], [0.3, 0.5, 0.3]);
  ...
  const finalAccentOpacity = useTransform([accentGradientOpacity, mountedValue], (values: number[]) => 
    values[1] === 0 ? 0.3 : values[0]
  );
  const finalGridOpacity = useTransform([gridOpacity, mountedValue], (values: number[]) => 
    values[1] === 0 ? 0.05 : values[0]
  );
  const finalVideoOpacity = useTransform([videoOpacity, mountedValue], (values: number[]) => 
    values[1] === 0 ? 0 : values[0]
  );
  const finalVideoScale = useTransform([videoScale, mountedValue], (values: number[]) => 
