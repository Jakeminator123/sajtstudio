    ...
    return (
        ...
        >
            ...
            <motion.div className="absolute inset-0 pointer-events-none" suppressHydrationWarning>
                {[...Array(20)].map((_, i) => {
                    // Use deterministic values for SSR, will be overridden on client
                    const randomLeft = typeof window !== 'undefined' ? Math.random() * 100 : i * 5;
                    const randomTop = typeof window !== 'undefined' ? Math.random() * 100 : i * 5;
                    const randomScale = typeof window !== 'undefined' ? 1 + Math.random() : 1.5;
                    const randomX = typeof window !== 'undefined' ? (Math.random() - 0.5) * 200 : 0;
                    const randomY = typeof window !== 'undefined' ? (Math.random() - 0.5) * 200 : 0;
