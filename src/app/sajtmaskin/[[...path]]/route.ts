  ...
  try {
    ...
    if (contentType.includes("text/html")) {
      ...
      content = content.replace(
        ...
        `${request.nextUrl.origin}/sajtmaskin`
      );
      ...
      content = content.replace(
        ...
        (match, url) => {
          if (url.startsWith(SAJTMASKIN_URL)) {
            return `href="${url.replace(SAJTMASKIN_URL, `${request.nextUrl.origin}/sajtmaskin`)}"`;
          }
          return match;
        }
      );
      ...
      content = content.replace(
        ...
        (match, url) => {
          if (url.startsWith(SAJTMASKIN_URL)) {
            return `src="${url.replace(SAJTMASKIN_URL, `${request.nextUrl.origin}/sajtmaskin`)}"`;
          }
          return match;
        }
      );
