...
function validateAuditResult(result: unknown): result is AuditResult {
  ...
  if (typeof cost.tokens !== 'number' || typeof cost.sek !== 'number' || typeof cost.usd !== 'number') {
    return false
  }
...
async function generatePDF(result: AuditResult) {
  ...
  try {
    ...
    console.log(`PDF generated in ${pdfDuration}ms, size: ${pdfBuffer.length} bytes`)
...
export async function POST(request: NextRequest) {
  ...
  const requestId = `pdf_${Date.now()}_${Math.random().toString(36).substring(7)}`

  try {
    let body
    try {
      body = await request.json()
      console.log(`[${requestId}] PDF request received`)
    } catch (parseError) {
    ...
    console.log(`[${requestId}] PDF request completed in ${totalDuration}ms`)
    ...
    const totalDuration = Date.now() - requestStartTime
    console.error(`[${requestId}] PDF generation error after ${totalDuration}ms:`, {
      error: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
    })
