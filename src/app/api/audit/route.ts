...
import { ALLOWED_MODEL_IDS, DEFAULT_MODEL, getFallbackModel } from '@/config/openai'
import { scrapeWebsite } from '@/lib/webscraper'
import { promises as fs } from 'fs'
import { NextRequest, NextResponse } from 'next/server'
import { OpenAI } from 'openai'
import path from 'path'
import type { AuditResult, Improvement, TechnicalRecommendation } from '@/types/audit'
...
const PRICE_IN_PER_MTOK = parseFloat(process.env.OPENAI_PRICE_IN_PER_MTOK_USD || '1.25')
const PRICE_OUT_PER_MTOK = parseFloat(process.env.OPENAI_PRICE_OUT_PER_MTOK_USD || '10.0')
...
function generateMarkdown(result: AuditResult): string {
  ...
  markdown += `**Datum:** ${new Date(result.timestamp || new Date()).toLocaleDateString('sv-SE')}\n`
  ...
  if (result.audit_scores) {
    ...
    for (const [key, value] of Object.entries(result.audit_scores)) {
      const name = key.replace(/_/g, ' ').charAt(0).toUpperCase() + key.replace(/_/g, ' ').slice(1)
  ...
  if (result.competitor_benchmarking) {
    ...
    if (
      ...
      result.competitor_benchmarking.industry_leaders.forEach((leader: string) => {
        markdown += `  - ${leader}\n`
      })
    }
    if (
      ...
      result.competitor_benchmarking.common_features.forEach((feature: string) => {
        markdown += `  - ${feature}\n`
      })
    }
    if (
      Array.isArray(result.competitor_benchmarking.differentiation_opportunities) &&
  ...
  if (result.priority_matrix) {
    ...
    if (result.priority_matrix.major_projects && result.priority_matrix.major_projects.length > 0) {
    ...
    if (result.priority_matrix.fill_ins && result.priority_matrix.fill_ins.length > 0) {
  ...
  if (result.technical_architecture) {
    ...
    if (result.technical_architecture.recommended_stack) {
      ...
      Object.entries(result.technical_architecture.recommended_stack).forEach(([key, value]) => {
        markdown += `  - ${key}: ${value}\n`
      })
    }
    if (
      ...
      result.technical_architecture.integrations.forEach((integration: string) => {
    ...
    if (
      ...
      result.technical_architecture.security_measures.forEach((measure: string) => {
        markdown += `  - ${measure}\n`
      })
    }
    markdown += '\n'
  ...
  if (result.implementation_roadmap) {
    ...
    ;['phase_1', 'phase_2', 'phase_3', 'launch'].forEach((phase) => {
      if (roadmap[phase]) {
        ...
        if (phaseData.duration) markdown += `- **Tid:** ${phaseData.duration}\n`
  ...
  if (result.success_metrics) {
    ...
    if (Array.isArray(result.success_metrics.kpis) && result.success_metrics.kpis.length > 0) {
      markdown += `- **KPI:er:**\n`
...
async function pruneOldAuditFiles(directory: string, maxFiles = MAX_PERSISTED_AUDITS) {
  try {
    ...
    const filesToDelete = filesWithMeta.sort((a, b) => b.mtime - a.mtime).slice(maxFiles)
...
function validateAuditResult(result: unknown): result is AuditResult {
  ...
  if (!r.audit_type || !['website_audit', 'recommendation'].includes(r.audit_type as string)) {
    return false
  }
...
export async function POST(request: NextRequest) {
  ...
  let requestId = `req_${Date.now()}_${Math.random().toString(36).substring(7)}`
  ...
  try {
    ...
    const { mode, url, answers, model = DEFAULT_MODEL, webSearch = false } = body
    ...
    if (mode === 'audit') {
      // Website audit mode
      if (!url) {
        return NextResponse.json({ error: 'URL krävs för audit' }, { status: 400 })
      }
      ...
      try {
        ...
        const scrapeDuration = Date.now() - scrapeStartTime
        console.error(`[${requestId}] Scraping error after ${scrapeDuration}ms:`, {
      ...
      // Questions mode
      if (!answers) {
        return NextResponse.json({ error: 'Svar krävs för rekommendationer' }, { status: 400 })
      }
    ...
    try {
      ...
      try {
        ...
        promptArray.forEach((msg, idx) => {
          if (!msg || typeof msg !== 'object') {
            throw new Error(`Invalid prompt format at index ${idx}: message is not an object`)
          }
          if (!msg.role || !['system', 'user'].includes(msg.role)) {
            throw new Error(`Invalid prompt format at index ${idx}: invalid role "${msg.role}"`)
          }
          ...
          const combinedContent = msg.content
            .map((c, cIdx) => {
              if (typeof c !== 'object' || c === null) {
                throw new Error(`Invalid content item format at message ${idx}, content ${cIdx}`)
              }
              if (c.type !== 'text') {
                throw new Error(
              ...
              if (typeof c.text !== 'string') {
                throw new Error(`Content text is not a string at message ${idx}, content ${cIdx}`)
              }
              return c.text
            })
            .join('\n')
      ...
      try {
        ...
        debugLog(
          `OpenAI API call completed in ${elapsedTime}ms (${(elapsedTime / 1000).toFixed(1)}s)`
        )
      ...
      debugLog('OpenAI API response received:', {
        hasOutput:
          !!(response as { output_text?: string; output?: unknown })?.output_text ||
          !!(response as { output?: unknown })?.output,
        usage: (response as { usage?: unknown })?.usage,
        status: (response as { status?: unknown })?.status,
        incompleteDetails: (response as { incomplete_details?: unknown })?.incomplete_details,
      })
      ...
      try {
        ...
        console.warn('Direct JSON parse failed, attempting to extract JSON object:', {
        ...
        if (!jsonString) {
          console.error('Failed to parse AI response (no JSON found):', {
            ...
            outputTextEnd: outputText.substring(Math.max(0, outputText.length - 200)),
          })
          return NextResponse.json(
            {
              error: 'Kunde inte tolka AI-svaret - inget JSON hittades i svaret',
            },
            { status: 500 }
          )
        }
      ...
      const costUSD =
        (inputTokens * PRICE_IN_PER_MTOK + outputTokens * PRICE_OUT_PER_MTOK) / 1_000_000
      ...
      if (!validateAuditResult(auditResultWithCost)) {
        const validationDetails = {
          ...
          hasIssues:
            Array.isArray(auditResultWithCost.issues) && auditResultWithCost.issues.length > 0,
          resultKeys: Object.keys(auditResultWithCost),
        }

        console.error(`[${requestId}] Result failed validation:`, {
          ...validationDetails,
          auditResult: JSON.stringify(auditResultWithCost, null, 2).substring(0, 1000),
        })
        ...
        if (
          ...
          missingFields.push('company, improvements, audit_scores, strengths eller issues')
        }

        return NextResponse.json(
          {
            error: `Kunde inte tolka AI-svaret - resultatet saknar nödvändiga fält. ${
              missingFields.length > 0 ? `Saknar: ${missingFields.join(', ')}` : ''
            }`,
          },
          { status: 500 }
        )
      ...
      try {
        ...
        const filePrefix =
          mode === 'audit'
            ? `audit_${result.domain?.replace(/\./g, '_') || 'unknown'}_${dateStr}`
        ...
        const saveDuration = Date.now() - saveStartTime
        console.error(`[${requestId}] Failed to save audit results after ${saveDuration}ms:`, {
      ...
      console.error(`[${requestId}] OpenAI API error after ${totalDuration}ms:`, {
        message: err.message,
        status: err.status,
        code: err.code,
        type: err.type,
      ...
      if (err.status === 401) {
        ...
        errorMessage = 'OpenAI-tjänsten är tillfälligt otillgänglig. Försök igen om en stund.'
