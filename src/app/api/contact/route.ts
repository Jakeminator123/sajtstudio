    ...
    try {
      ...
      if (error) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('Contact form submission (email failed):', { name: trimmedName, email: trimmedEmail, message: trimmedMessage });
          return NextResponse.json(
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('Contact form submission (email failed, dev mode):', { name: trimmedName, email: trimmedEmail, message: trimmedMessage });
        return NextResponse.json(
          { success: true, message: "Meddelandet har skickats!" },
          { status: 200 }
        );
