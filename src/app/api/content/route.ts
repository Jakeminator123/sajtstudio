...
export async function GET(request: NextRequest) {
  try {
    ...
    if (key) {
      ...
      if (!content) {
        return NextResponse.json({ error: `Content not found: ${key}` }, { status: 404 })
      }
      return NextResponse.json({
        success: true,
        content,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!updated) {
      return NextResponse.json({ error: `Unknown content key: ${key}` }, { status: 404 })
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Invalid action. Use 'seed' or 'reset'" }, { status: 400 })
  } catch (error) {
